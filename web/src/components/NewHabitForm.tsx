...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if (weekDays.includes(weekDay)) {
      ...
      const weekDayWithEditedOne = [...weekDays, weekDay]
      setWeekDays(weekDayWithEditedOne)
    }
  }
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
        ...
        {availableWeekDays.map((weekDay, index) => {
          return (
            ...
            >
              ...
              <span className='text-white leading-tight focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900'>
                {weekDay}
              </span>
            </Checkbox.Root>
          )
      ...
      <button
      type="submit"
      className="mt-6 rounded-lg p-4 flex items-center justify-center gap-3 font-semibold bg-green-600 transition-colors hover:bg-green-500">
